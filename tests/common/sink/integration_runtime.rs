    ...
    pub async fn run(&self, clear_component: bool) -> Result<()> {
        ...
        for (idx, sink_info) in self.sink_infos.iter().enumerate() {
            ...
            let mut sink = sink_info
                ...
                .build(&spec, &ctx)
                .await
                .map_err(|err| anyhow::anyhow!("创建 Sink 失败: {err}"))?;
            ...
            sink.sink
                .sink_records(batch_records)
                .await
                .map_err(|err| anyhow::anyhow!("发送测试数据失败: {err}"))?;
            ...
            let mut sink = sink_info
                ...
                .build(&spec, &ctx)
                .await
                .map_err(|err| anyhow::anyhow!("重建 Sink 失败: {err}"))?;
            ...
            sink.sink
                .sink_records(retry_batch_records)
                .await
                .map_err(|err| anyhow::anyhow!("重启后发送测试数据失败: {err}"))?;
